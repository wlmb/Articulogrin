...
\notaL{\sout{ref}}. The potential within the electrolytic cell coincides
...
\notaL{\sout{Me tienes que explicar dónde y cómo usaste lo que mencionas en
  la siguiente frase:
...
$n_Hd_H=\lambda_c/2$ \notaC{$n_Ld_L=\lambda_c/2$ error y
 descuido de escritura.} and the MC has the
following sequence: $(HL)_6HH(LH )_6$. \notaL{\sout{Hay algo mal. La
...
electrode at $\bm
r_0=(x_0,y_0,z_0)=(0.1\text{cm},0.3\text{cm},0.9\text{cm})$ \notaC{ya lo corregi 
$0.1\text{cm},0.3\text{cm},0.9\text{cm})$ }.
\notaL{\sout{Descuidado. En la figura dice 0.98, no 0.96}}.
\notaL{\sout{¿Por qué usaste estos valores?}}
\notaL{\sout{Esto no es interesante: the programming with the
  PERL PDL package and corresponding graph in gnuplot.}}
\notaL{\sout{¿Qué significa esto?:We optimize the
appropriate values so that our current density adequately attacks
crystalline silicon, giving the gradient effect.}} \notaC{Yo quise volver a 
reafirmar algo, pero se pierde el sentido real de la frease:"optimizamos los
valores apropiados de fabricación para que nuestra densidad actual, atacará
silicio cristalino, dando el efecto gradiente al silicio poroso"}
For these parameters we obtained $ j_\perp/I\sim
0.5-1.2\text{cm}^{-2}$
\notaL{\sout{Quité el resto del párrafo pues no entendí nada.}}
...
$p_C= 0.40$. The depth $ d_A= 1320nm $, $ d_B= 997nm $ and $ d_C= 605nm $ 
respectively. \notaC{Las profundidades $ d_A= 1320nm $, $ d_B= 997nm $ 
and $ d_C= 605nm $ }
\notaL{\sout{Falta decir cuáles fueron las profundidades}}
Location $A$ is

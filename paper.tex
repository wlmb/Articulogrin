...
\begin{tabular}{lrrrrrr}
  ...
  $I$ (mA) &$x'_n$ (cm)&  $j$ ($\text{mA}/\text{cm}^2$) & $p$
       & $d$ ($\mu\text{m}$) & $s$ & $v$ (nm/s)\\
\hline
$S_1$/ 5 &0.2 & 3.61 & 0.494 & 0.340 & 0.849&  1.361 \\
...
$S_2$/10 &0.2 & 7.22 & 0.591 & 0.635 & 0.856&  2.539 \\
...
$S_3$/20 &0.2 &14.45 & 0.686 & 1.204 & 1.149&  4.818 \\
...
\begin{figure}
  ...
  \caption{Porosity vs. current density for the three samples
    $S_1$, $S_2$, $S_3$. The data points for each sample are joined by
    thick lines. The arrows point to data that correspond to
    position $x'_1=0.2\,\text{cm}$, joined to the other data by thin
    lines (see text). We include linear fits of the form
    $p=mj_\perp+b$ to the data from
    sample $S_3$ (thick straight line, with parameters $m=0.0134$ and
    $b=0.5288$) and to that from all the samples (thin straight line). }
  \label{fig:p}
\end{figure}
...
analysis. We made a linear fit of the form $p=mj_\perp+b$ to the data from sample $S_3$ and to
the data from the three samples. A comparison shows that a fit to the
data of just one sample, $S_3$, turns out to be a reasonable predictor for the
...
\begin{figure}
  ...
  \caption{Etching rate vs. current density for three samples
    ...
    position $x'_1=0.2\,\text{cm}$, joined to the other data by thin
    lines (see text). We include linear fits of the form
    $v=mj_\perp+b$ to the data from
    sample $S_3$ (thick straight line, with parameters $m=0.3543$,
    $b=0.0924$) and to that from all the samples (thin straight line). }
  \label{fig:v}
\end{figure}
...
samples. Furthermore, as the results corresponding to sample $S_2$ in

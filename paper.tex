...
We concentrate our attention only on the region $0\le z<z_0$. From
...
sides $a=2.03\,\text{cm}$ and $b=1.48\,\text{cm}$, with an uncovered sample area
...
parameters, and thus, we related the porosity
$p$ and the etching rate $v=d/t$ to the local current density $j_\perp$.

\ifexclude
\notaL{Quitar esta sección:
...
\begin{figure}
  ...
  \caption{Normalized current density $j_\perp(x,y,0)/I$ calculated a
    ...
    length $a = 2.03\,\text{cm}$ and width $b=1.48\,\text{cm}$, with
...
for cells with aspect ratios closer to 1.

\subsection{PS GRIN Films }
...
\begin{figure}
 \centering
  ...
  \caption{ Photograph of sample $G_3$ prepared as described in
    ...
    $x_1=0.2\text{cm}$,  $x_2=0.4\text{cm}$, $x_3=0.6\text{cm}$,
    $x_4=0.8\text{cm}$, $x_5=1\text{cm}$, $x_6=1.2\text{cm}$,
    $x_7=1.4\text{cm}$, $x_8=1.6\text{cm}$, with $y_n=0.74\,\text{cm}$
    on which we measured reflectance spectra. During the preparation the cathode was over
    the leftmost point $x_1$.}
  \label{fig:SI3}
\end{figure}
...
same conditions as those corresponding to Fig. \ref{fig:SI3} applying a current
...
neighborhood of the wall. Thus, we may expect our formula \eqref{Eq:J}
to fail close to the border, at distances of the order of the radius
of the cross-section of the o-ring.

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{R20vslambda}
  \caption{Reflectance spectra at different positions $(x,y)$
    on sample $G_3$ (shown in Fig. \ref{fig:SI3}), where
    $x=0.2-1.6\,\text{cm}$ and $y=0.74\,\text{cm}$. We show
...
\end{figure}
 ...
 structures, $G_3$, prepared as described in
 \ref{sec:fabrication-ps-grin} applying a current $I_3=20\,\text{mA}$
 for a time $t=250\,\text{s}$. We employed the same geometry as that described in
 Sec. \ref{sec:current-density}
The positions $\bm R_n=(x_n,y_n)$, $n=1\ldots8$  with
$x_n=0.2n\,\text{cm}$ and $y_n=0.74\,\text{cm}$ on which reflectance
spectra were measured are indicated. The electrode was situated above
the position $\bm R_1$  corresponds to the left side of the

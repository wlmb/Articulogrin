...
with very-large-scale-integration (VLSI) devices \hl{that may be
...
the substrate to produce the PS sample. \hl{As expected, the current
...
\begin{figure}
  ...
  \caption{\hl{(a):} Schematic experimental set-up for
    ...
    lines. \hl{(b) and (c):} Top and side schematic views of expected resulting sample
...
We manufactured three samples \hl{$S_1$, $S_2$ and $S_3$} by applying a
...
\begin{figure}
  ...
  \label{fig:DR1}
\end{figure}
In Fig. \ref{fig:DR1} we show the \hl{normalized} current density $j_\perp(x,y,0)/I$ calculated through
...
\begin{figure}
 ...
 \includegraphics[width=\textwidth]{G3I20proc}
  \caption{ Photograph of sample \hl{$S_3$} prepared as described in
...
the etched sample, \hl{which corresponds to} the position of the sealing o-ring,
...
Fig. \ref{fig:SI3}. \hl{We included an estimated uncertainty $\Delta
  x_n=\Delta x$ in the
...
In Fig. \ref{fig:sem} we show some SEM images of the sample \hl{$S_3$,}
...
\begin{figure}
  ...
  \caption{Top ((a), (b), and (c)) and lateral ((d), (e), and (f)) SEM
    micrographs of the sample \hl{$S_3$} taken from a region corresponding to the left
...
(f)). \hl{We remark that these micrographs were taken at three positions
...
determination of neither the thickess nor the porosity; they were used
...
film  $d$ as
adjustable parameters. \hl{To that end we used the {\tt migrad} routine of
...
\caption{\label{t:jpdv}
  \hl{Current density and fitted parameters for several positions on
  three samples:  porosity, thickness, and etching rate.}}
\end{table}
\hl{The uncertainties in the fitted parameters $p$, $d$, and $v$ were
chosen as those that increase the sum of squared errors by 10\% with
respect to its minimum value.}

From the data in Table \ref{t:jpdv} \hl{we can plot the fitted porosity vs.
...
\begin{figure}
  ...
  \caption{\hl{Porosity vs. current density for the three samples
...
analysis. \hl{We made a linear fit of the form $p=mj_\perp+b$ obtaining
...
\begin{figure}
  ...
  \label{fig:v}
\end{figure}
\hl{As for the porosity, excluding the points in the
...
\begin{equation}\label{eq:v}
  \hl{v\approx (0.312\pm0.017)\frac{\text{cm}^2}{\text{mA}}
...
\begin{figure}
  ...
  \caption{Reflection $R$ of a PS film as a function of the current density
    ...
    data corresponding to the corresponding samples \hl{$S_1$, $S_2$
...
samples. \hl{Furthermore, as discussed above and for visualization
...
rates corresponding to each current density. \hl{We used these data to
...
\hl{a procedure that from just one or a few samples can produce calibration curves
